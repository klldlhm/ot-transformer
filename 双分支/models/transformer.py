...
class Transformer(nn.Module):
    ...
    def forward(self, style, mask, content):
        # content-aware positional embedding
        content_pool = self.averagepooling(content)
        pos_embed_c = F.interpolate(self.new_ps(content_pool), mode='bilinear',
                                    size=style.shape[-2:])

        # style-aware positional embedding
        style_pool = self.averagepooling(style)
        pos_embed_s = F.interpolate(self.new_ps(style_pool), mode='bilinear',
                                    size=style.shape[-2:])
